...
import { createStripeCheckoutSession, createPayPalOrder } from '../../api/stripe';
import { getAvailableMethods } from '../../api/settings';
...
const PaymentMethodSelector: React.FC<{
...
}> = ({ selected, onSelect, availableMethods }) => {
    const methods = [
    ...
    if (visibleMethods.length <= 1) return null;
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, item, bookingDetails, onBookingSuccess }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            getAvailableMethods('IT') // Assume user country is IT
                .then(methods => {
                ...
                .catch(err => {
                    console.error("Failed to fetch available payment methods:", err);
                    setAvailableMethods([]);
                })
                .finally(() => {
    ...
    const depositRequiresStripe = (item.securityDeposit || 0) > 0 && !availableMethods.includes('card');
    ...
    const handlePayment = async () => {
        ...
        if (!hubber) {
            alert("Errore: Hubber non trovato.");
            setIsLoading(false);
            return;
        }
        
        const newBookingId = MOCK_BOOKINGS.length > 0 ? Math.max(...MOCK_BOOKINGS.map(b => b.id)) + 1 : 1;
        ...
        try {
            const idempotencyKey = `checkout-${pendingBooking.id}-${Date.now()}`;
            if (activeMethod === 'paypal') {
                const { url } = await createPayPalOrder(newBookingId, idempotencyKey);
                window.location.href = url;
            } else { // 'card' or 'klarna'
                const allowKlarna = activeMethod === 'klarna';
                const { url } = await createStripeCheckoutSession(newBookingId, allowKlarna, idempotencyKey);
                window.location.href = url;
            }
        } catch (error) {
            console.error("Failed to create payment session", error);
            const failedBooking: Booking = { ...pendingBooking, status: BookingStatus.PAYMENT_FAILED };
            onBookingSuccess(failedBooking);
        ...
        <>
            <div className="p-6">
                ...
                {isLoadingMethods ? (
                    ...
                    <>
                        <PaymentMethodSelector selected={activeMethod} onSelect={setActiveMethod} availableMethods={availableMethods} />

                        {activeMethod === 'card' && availableMethods.includes('card') && <MockCardForm formId="main" />}
                        {activeMethod === 'paypal' && availableMethods.includes('paypal') && <p className="text-center text-gray-500 p-8">Verrai reindirizzato a PayPal per completare il pagamento.</p>}
                        {activeMethod === 'klarna' && availableMethods.includes('klarna') && (
                            <div>
                    ...
                    <div className="p-4 text-center bg-red-50 text-red-700 rounded-lg">
                        <p className="font-semibold">Nessun metodo di pagamento configurato.</p>
                        <p className="text-sm">Contattare l'amministratore per abilitare i pagamenti.</p>
                    </div>
                )}
            </div>

            <footer className="p-6 border-t bg-gray-50 rounded-b-2xl">
                {depositRequiresStripe && (
                    <p className="text-xs text-red-600 text-center mb-2">Il deposito cauzionale richiede Stripe (pagamento con carta), che non è al momento abilitato. Impossibile procedere.</p>
                )}
                <button
                    onClick={handlePayment}
                    disabled={isLoading || isLoadingMethods || availableMethods.length === 0 || depositRequiresStripe}
